...
class CyclicList:
    ...
    def __iter__(self):
        while True:
            yield self.iterable[self.count % len(self)]
            self.count += 1
    ...
    def __getitem__(self, _index):
        return self.iterable[_index % len(self.iterable)]

    def append(self, item):
        self.iterable.append(item)

    def pop(self, _index=None):
        if _index is not None:
            return self.iterable.pop(_index % len(self.iterable))
